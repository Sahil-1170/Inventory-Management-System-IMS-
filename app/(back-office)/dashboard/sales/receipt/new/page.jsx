...
export default function NewReceipt({ initialData = {}, isUpdate = false }) {
  ...
  function redirect(){
    router.push("/dashboard/sales/receipt")
  }
  async function onSubmit(data) {
    ...
    if(isUpdate){
      // Update Request
      makePutRequest(setLoading,`${baseUrl}/api/receipt/${initialData.id}`, data, "Receipts", redirect, reset)
    }else
    makePostRequest(setLoading, `${baseUrl}/api/receipt`, data, "Receipts", reset)
  }
  return (
    <div>
      {/* Header */}
      <FormHeader title={isUpdate?"Update Sales Receipts":"New Sales Receipts"} href="/dashboard/sales/receipt" />
      {/* Form */}
      <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-4xl p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700 mx-auto my-3">
        <div className="grid gap-4 sm:grid-cols-2 sm:gap-6">
          ...
          <TextInput label="Payment Id" name="paymentId" register={register} errors={errors} className="w-full" placeholder="Type the Payment Id" />
        ...
        <SubmitButton isLoading={loading} title={isUpdate?"Updated Sales Receipts":"New Sales Receipts"} />
      </form>
    </div>
  )
}
