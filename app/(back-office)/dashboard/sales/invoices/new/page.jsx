...
export default function NewInvoices({ initialData = {}, isUpdate = false }) {
  ...
  async function onSubmit(data) {
    ...
    if(isUpdate){
      // Update Request
      makePutRequest(setLoading,`${baseUrl}/api/invoices/${initialData.id}`, data, "Invoices", redirect, reset)
    }else
    makePostRequest(setLoading, `${baseUrl}/api/invoices`, data, "Invoices", reset)
  }
  return (
    <div>
      {/* Header */}
      <FormHeader title={isUpdate?"Update Invoices":"New Invoices"} href="/dashboard/sales/orders" />
      {/* Form */}
      <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-4xl p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700 mx-auto my-3">
        ...
        <SubmitButton isLoading={loading} title={isUpdate?"Updated Invoices":"New Invoices"} />
      </form>
    </div>
  )
}
