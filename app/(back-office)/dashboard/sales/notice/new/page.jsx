...
export default function NewNotice({ initialData = {}, isUpdate = false }) {
  ...
  function redirect(){
    router.push("/dashboard/sales/notice")
  }
  async function onSubmit(data) {
    ...
    if(isUpdate){
      // Update Request
      makePutRequest(setLoading,`${baseUrl}/api/notice/${initialData.id}`, data, "Notice", redirect, reset)
    }else
    makePostRequest(setLoading, `${baseUrl}/api/notice`, data, "Notice", reset)
  }
  return (
    <div>
      {/* Header */}
      <FormHeader title={isUpdate?"Update Notice":"New Notice"} href="/dashboard/sales/return" />
      {/* Form */}
      <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-4xl p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700 mx-auto my-3">
        ...
        <SubmitButton isLoading={loading} title={isUpdate?"Updated Notice":"New Notice"} />
      </form>
    </div>
  )
}
